...
export async function handleReactionAdd(
  ...
  switch (emojiIdentifier) {
    case '%E2%9A%A0%EF%B8%8F': //⚠""
      if (checkIfMod(user.id)) {
        ...
        if (
          !checkIfDuplicate(
            warningSystem[messageAuthorId].warnedMessages,
            messageId,
          )
        ) {
          messageReaction.message.channel.send(
            `<@${messageAuthorId}> a moderator has flagged your message with a warning`,
          );
  ...
  // REGION EDITING IS NO LONGER SUPPORTED IN DISCORDJS API
  //   if (messageId == regionMessageId && user.id != Config.botID) {
  //     switch (emojiIdentifier) {
  //       case '%F0%9F%87%AA': //E
  //         await message.guild.edit({ region: 'us-east' });
  //         message.channel.send('Server region has been changed to **US-EAST**');
  //         break;
  //       case '%F0%9F%87%BC': //W
  //         await message.guild.edit(
  //           { region: 'us-west' },
  //           `Requested by Moderator ${messageReaction.message.author.username}`,
  //         );
  ...
  //           'Server region has been changed to **US-CENTRAL**',
  //         );
  //         break;
  //       case '%F0%9F%87%B8': //S
  //         await message.guild.edit(
            ...
            message.mentions.members.forEach((user) => {
              user
                .send(
                  'Sorry ' +
                    user.displayName +
                    ' you have been pruned from XWest due to inactivity. Please reach out to an admin or member if you would like to be readded.',
                )
                .then(() => {
                  user.kick();
                });
        ...
        switch (command) {
          case ServerCommands.region:
            ...
            await message.channel.send(
              'Changing Regions is no longer possible under new bot API',
            );
            //   const regionMessage = await message.channel.send(
            //     `Current region **${message.guild.region.toUpperCase()}**\nTo change react below:\n    **🇼:US-WEST   🇪:US-EAST   🇨:US-CENTRAL   🇸:US-SOUTH**`,
            //   );
