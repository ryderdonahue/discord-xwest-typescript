...
async function main() {
  ...
  client.on('ready', async () => {
    ...
    const XWestServer = client.guilds.cache.find(
      (guild) => guild.id === Config.xwestServerId,
    );
    const RoleChannel = await XWestServer.channels.cache.find(
      (channel) => channel.id === Config.roleChannelId,
    );
  ...
  // sending messages no longer permitted
  // WELCOME MESSAGE FOR NEW MEMBERS
  //   client.on('guildMemberUpdate', async function (
  //     oldMember: Discord.GuildMember,
  //     newMember: Discord.GuildMember,
  //   ) {
  //     if (
  //       !oldMember.roles.cache.has(Config.memberRoleId) &&
  //       newMember.roles.cache.has(Config.memberRoleId)
  //     ) {
  //       //   newMember.send(
  ...
  client.on('messageCreate', CreditManagement.handleMessage);
  ...
  client.on('messageCreate', ModeratorTools.handleMessage);

  // BOT BEHAVIOR
  client.on('messageCreate', BotBehavior.handleMessage);
  client.on('messageReactionAdd', BotBehavior.handleReactionAdd);

  // REPOST MANAGEMENT
  client.on('messageCreate', RepostManagement.handleMessage);
}
